  ...
  <div class="table-enter-next-example">
    ...
    <el-card class="example-card">
      ...
      >
        ...
        <el-table-column label="地址" prop="address">
          <template #default="scope">
            <el-select clearable v-model="scope.row.gender" placeholder="请选择性别">
              <el-option label="男" value="男"></el-option>
              <el-option label="女" value="女"></el-option>
            </el-select>
          </template>
    ...
    <el-card class="example-card">
      ...
      <ol>
        ...
        <li>如果是整个表格中的最后一个输入元素，会触发no-next-input事件</li>
      </ol>
    </el-card>
  </div>
</template>
...
// 处理删除指定行
const handleDelete = (row: TableRowData, index: number) => {
...
// 当没有下一个输入元素时的处理
const handleNoNextInput = ({ rowIndex }: noNextInputParams) => {
